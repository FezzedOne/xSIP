...
function sip.setDrawableIcon(wid, path, drawable, directives)
  if path then
    ...
    widget.setImage(wid, "/assetmissing.png")
  end
end
...
function sip.randomizeItem()
  if sip.item then
    local params = sip.item.parameters or sip.getSpawnItemParameters(root.itemConfig(sip.item.name).config)
...
function sip.showSpecifications(itemConfig, parameters)
  local pane = nil
  ...
  -- Blueprint
  widget.setButtonEnabled(sip.widgets.blueprint, (itemConfig and not parameters) and sip_util.hasBlueprint(sip.item.name))
  -- Upgrade
  widget.setButtonEnabled(sip.widgets.upgrade, (itemConfig and not parameters) and sip_util.isUpgradeable(itemConfig))
