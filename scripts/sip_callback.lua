...
function sip.callback.selectItem()
    ...
    -- Show text
    widget.setText(
        sip.widgets.itemName,
        (sip.item.parameters or {}).shortdescription or config.shortdescription or sip.item.name
    )
    widget.setText(
        sip.widgets.itemDescription,
        (sip.item.parameters or {}).description or config.description or sip.lines.descriptionMissing
    )
    ...
    -- Item slot
    sip.randomizeItem()
    sip.showSpecifications(config, sip.item.parameters)
end
...
function sip.callback.selectWeaponElement(_, data) sip.randomizeItem() end

function sip.callback.randomizeItem() sip.randomizeItem() end
...
function sip.callback.takeItem()
    ...
    if not swapItem then
        ...
        widget.setText(
            sip.widgets.itemName,
            swapItem.parameters and swapItem.parameters.shortdescription or sip.lines.copiedItem
        )
        widget.setImage(
            sip.widgets.itemRarity,
            sip.rarityImages.flags[swapItem.parameters and swapItem.parameters.rarity or "common"]
        )

        sip.showSpecifications(nil, nil)
    end
end
...
function sip.callback.showType(_, t)
    ...
    if not sip.knownCategories[t] then
        sb.logError("SIP: Could now show items for the type '%s'.", t)
        return
    end
...
function sip.callback.openEditor()
    ...
    --loads the api
    if not itemeditork then pcall(require, "/api/itemeditork.lua") end
